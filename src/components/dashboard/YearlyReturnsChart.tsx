...
export function YearlyReturnsChart() {
  ...
  const maxVal = Math.max(
    ...yearlyReturns.map((y) => Math.abs(y.return)),
    ...yearlyReturns.map((y) => Math.abs(y.btcReturn ?? 0)),
    1
  );
                ...
                <div className="relative h-6 flex-1 rounded-sm bg-bg-elevated overflow-hidden">
                  <div
                    ...
                    className={cn(
                      ...
                      y.return >= 0 ? "left-1/2 bg-gold/70" : "right-1/2 bg-pnl-negative/50"
                    )}
                    style={{ width: `${(Math.abs(y.return) / maxVal) * 50}%` }}
                  />
              ...
              <div className="flex items-center gap-2">
                <div className="relative h-6 flex-1 rounded-sm bg-bg-elevated overflow-hidden">
                  <div
                    ...
                    className={cn(
                      "absolute top-0 h-full rounded-sm transition-all duration-500",
                      (y.btcReturn ?? 0) >= 0 ? "left-1/2 bg-text-muted/40" : "right-1/2 bg-pnl-negative/30"
                    )}
                    style={{ width: `${(Math.abs(y.btcReturn ?? 0) / maxVal) * 50}%` }}
                  />
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="font-[family-name:var(--font-mono)] text-[11px] text-text-secondary/70">
                      {formatReturn(y.btcReturn ?? 0)}
                    </span>
                  </div>
                </div>
