...
type DisplayData = ReturnType<typeof useTearsheetDisplayData>;
...
/* ─── Monthly Heatmap ─── */
function MonthlyHeatmap({ d }: { d: DisplayData }) {
  ...
  (d.monthlyReturns ?? []).forEach((m: { year: number; month: string; value: number }) => {
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-xs">
        ...
        <tbody>
          {(d.yearlyReturns ?? []).map((yearData: { year: number; rebeta: number }) => {
...
function YearlyBars({ d }: { d: DisplayData }) {
  const yr = d.yearlyReturns ?? [];
    ...
    <div>
      ...
      <div className="p-6">
        ...
        <div className="mt-6 space-y-8">
          {tab === "overview" && <OverviewTab d={display} />}
          {tab === "returns" && <ReturnsTab d={display} />}
          {tab === "risk" && <RiskTab d={display} />}
          {tab === "benchmark" && <BenchmarkTab d={display} />}
        </div>

        {/* Footer — dynamic data range */}
        <div className="mt-10 border-t border-border-subtle pt-4 text-center">
          <p className="text-[10px] text-text-muted">
            Live from daily_returns DB &middot; Period: {ts?.dataRange?.start ?? "..."} ~ {ts?.dataRange?.end ?? "..."} ({ts?.dataRange?.days ?? "..."} days) &middot; Benchmark: BTC
          </p>
        </div>
      </div>
    </div>
...
function OverviewTab({ d }: { d: DisplayData }) {
  return (
    <>
      ...
      <MetricTable title="Main Metrics" data={d.mainMetrics ?? []} />
...
function ReturnDistribution({ d }: { d: DisplayData }) {
...
function RiskTab({ d }: { d: DisplayData }) {
  return (
    <>
      {/* Underwater Chart */}
      <div>
      ...
      <div>
        ...
        <div className="mt-3 overflow-hidden rounded-sm border border-border-subtle bg-bg-card">
          <table className="w-full text-sm">
            ...
            <tbody>
              {(d.worstDrawdowns ?? []).map((dd: { rank: number; started: string; recovered: string; dd: number; days: number }) => (
                <tr key={dd.rank} className="border-b border-border-subtle last:border-0 transition-colors hover:bg-bg-elevated">
                  <td className="px-4 py-2 text-text-muted">{dd.rank}</td>
                  <td className="px-4 py-2 font-[family-name:var(--font-mono)] text-text-secondary">{dd.started}</td>
                  <td className="px-4 py-2 font-[family-name:var(--font-mono)] text-text-secondary">{dd.recovered}</td>
                  <td className="px-4 py-2">
                    <DrawdownDepthBar dd={dd.dd} maxDd={(d.worstDrawdowns ?? [{ dd: -1 }])[0].dd} />
                  </td>
                  <td className="px-4 py-2 text-right font-[family-name:var(--font-mono)] text-text-secondary">{dd.days}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div>
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 xl:grid-cols-4">
          {[
            { label: "Max Drawdown", rebeta: d.mainMetrics?.[6]?.rebeta ?? "--", btc: d.mainMetrics?.[6]?.btc ?? "—" },
            { label: "Longest DD", rebeta: d.mainMetrics?.[7]?.rebeta ?? "--", btc: d.mainMetrics?.[7]?.btc ?? "—" },
            { label: "Daily VaR (95%)", rebeta: d.returnsMetrics?.[0]?.rebeta ?? "--", btc: d.returnsMetrics?.[0]?.btc ?? "—" },
            { label: "CVaR (99%)", rebeta: d.returnsMetrics?.[2]?.rebeta ?? "--", btc: d.returnsMetrics?.[2]?.btc ?? "—" },
          ].map((s) => (
            <div key={s.label} className="rounded-sm border border-border-subtle bg-bg-card p-4">
...
function BenchmarkTab({ d }: { d: DisplayData }) {
  return (
    <>
      ...
      <MetricTable title="Benchmark Metrics (vs BTC)" data={d.benchmarkMetrics ?? []} />
        ...
        <div className="mt-3 grid gap-3 xl:grid-cols-2">
          ...
          <div className="rounded-sm border border-gold/30 bg-bg-card p-5">
            ...
            <div className="mt-4 space-y-3">
              {(d.mainMetrics ?? []).slice(0, 6).map((row: { metric: string; rebeta: string }) => (
          ...
          <div className="rounded-sm border border-border-subtle bg-bg-card p-5">
            ...
            <div className="mt-4 space-y-3">
              {(d.mainMetrics ?? []).slice(0, 6).map((row: { metric: string; btc: string }) => (
                <div key={row.metric} className="flex items-center justify-between">
                  <span className="text-xs text-text-secondary">{row.metric}</span>
